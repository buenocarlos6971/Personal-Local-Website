...
function begin_play(){
	...
	if(isEmpty(p1) || isEmpty(p2)){
		alert("Make sure to enter player names!")
	}
	else{
		if(!started){
			...
			this.started=true;
			if(whose_move()==1){
				document.getElementById("turn_info").innerHTML = document.getElementById("player1_id").value + " Turn" ;
			}
			else{
				document.getElementById("turn_info").innerHTML= document.getElementById("player2_id").value + " Turn";
			}
...
function reset_play(){
	...
	this.started=false;
	this.turn=1;
	for(var x=0; x<this.table_ids.length;x++){
		document.getElementById(table_ids[x]).innerHTML = table_ids[x];
		this.board_state[x]=-1;
	}
...
function play() {
	if(this.started){
		...
		if(!this.table_ids.includes(choice)){
			alert("That is not a valid move!");
		}
		else{
			if(this.board_state[this.table_ids.indexOf(choice)] == -1){
				if(whose_move()){
					document.getElementById(choice).innerHTML= "X";
					this.board_state[this.table_ids.indexOf(choice)]=1;
					document.getElementById("turn_info").innerHTML= document.getElementById("player2_id").value + " Turn";
					toggle_move();
				}
				else{
					document.getElementById(choice).innerHTML= "O";
					this.board_state[this.table_ids.indexOf(choice)]=0;
					document.getElementById("turn_info").innerHTML= document.getElementById("player1_id").value + " Turn";
					toggle_move();
				}
			who_won();
		...
		alert("Cannot play as game has not started")
	}
}

function who_won() {
	if(this.board_state[0] === 1 && this.board_state[1] === 1 && this.board_state[2] === 1){
			alert(document.getElementById("player1_id").value + " won the game");
			reset_play();
	}
 ...
 if(this.board_state[0] === 0 && this.board_state[1] === 0 && this.board_state[2] === 0){
			alert(document.getElementById("player2_id").value + " won the game");
			reset_play();
	}
